import getLazarSoftScanner from './lazarsoftscanner';
/**
 * @fileoverview
...
export default class Html5Qrcode {
  ...
  start(cameraIdOrConfig, configuration, qrCodeSuccessCallback, qrCodeErrorCallback) {
    ...
    if (config.videoConstraints) {
      if (!this._isMediaStreamConstraintsValid(config.videoConstraints)) {
        Html5Qrcode._logError("'videoConstraints' is not valid 'MediaStreamConstraints, " + "it will be ignored.'", /* experimental= */ true);
      } else {
    ...
    const foreverScan = () => {
      ...
      if ($this._localMediaStream) {
        ...
        $this._context.drawImage($this._videoElement, /* sx= */ sxOffset, /* sy= */ syOffset, /* sWidth= */ sWidthOffset, /* sHeight= */ sHeightOffset, /* dx= */ 0, /* dy= */ 0, /* dWidth= */ $this._qrRegion.width, /* dHeight= */ $this._qrRegion.height);
    ...
    return new Promise((resolve, /* ignore */ reject) => {
      ...
      const removeQrRegion = () => {
        while ($this._element.getElementsByClassName(Html5Qrcode.SHADED_REGION_CLASSNAME).length) {
          const shadedChild = $this._element.getElementsByClassName(Html5Qrcode.SHADED_REGION_CLASSNAME)[0];
  ...
  scanFile(imageFile, /* default=true */ showImage) {
    ...
    const computeCanvasDrawConfig = (imageWidth, imageHeight, containerWidth, containerHeight) => {
      if (imageWidth <= containerWidth && imageHeight <= containerHeight) {
        ...
        Html5Qrcode._log('Image downsampled from ' + `${formerImageWidth}X${formerImageHeight}` + ` to ${imageWidth}X${imageHeight}.`);
    ...
    return new Promise((resolve, reject) => {
      ...
      inputImage.onload = () => {
        ...
        const containerHeight = Math.max(element.clientHeight ? element.clientHeight : imageHeight, Html5Qrcode.FILE_SCAN_MIN_HEIGHT);

        const config = computeCanvasDrawConfig(imageWidth, imageHeight, containerWidth, containerHeight);
        if (showImage) {
          const visibleCanvas = $this._createCanvasElement(containerWidth, containerHeight, 'qr-canvas-visible');
          ...
          // More reference
          // https://developer.mozilla.org/en-US/docs/Web/API/CanvasRenderingContext2D/drawImage
          context.drawImage(inputImage, /* sx= */ 0, /* sy= */ 0, /* sWidth= */ imageWidth, /* sHeight= */ imageHeight, /* dx= */ config.x, /* dy= */ config.y, /* dWidth= */ config.width, /* dHeight= */ config.height);
        }
        ...
        context.drawImage(inputImage, /* sx= */ 0, /* sy= */ 0, /* sWidth= */ imageWidth, /* sHeight= */ imageHeight, /* dx= */ 0, /* dy= */ 0, /* dWidth= */ config.width, /* dHeight= */ config.height);
        try {
  ...
  applyVideoConstraints(videoConstaints) {
    ...
    return new Promise((resolve, reject) => {
      ...
      // directly.
      videoTrack
        .applyConstraints(videoConstaints)
        .then((_) => {
          resolve(_);
        })
        .catch((error) => {
  ...
  _isMediaStreamConstraintsValid(videoConstraints) {
    if (!videoConstraints) {
      Html5Qrcode._logError('Empty videoConstraints', /* experimental= */ true);
      return false;
    }

    if (typeof videoConstraints !== 'object') {
      const typeofVideoConstraints = typeof videoConstraints;
      Html5Qrcode._logError('videoConstraints should be of type object, the ' + `object passed is of type ${typeofVideoConstraints}.`, /* experimental= */ true);
      return false;
    }
    ...
    const bannedKeys = ['autoGainControl', 'channelCount', 'echoCancellation', 'latency', 'noiseSuppression', 'sampleRate', 'sampleSize', 'volume'];
    ...
    for (let i = 0; i < keysInVideoConstraints.length; i++) {
      ...
      if (bannedkeysSet.has(key)) {
        Html5Qrcode._logError(`${key} is not supported videoConstaints.`, /* experimental= */ true);
        return false;
      }
    }
