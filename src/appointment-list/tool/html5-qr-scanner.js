...
export default class Html5QrcodeScanner {
  ...
  render(qrCodeSuccessCallback, qrCodeErrorCallback) {
    ...
    this.qrCodeSuccessCallback = (message) => {
      ...
      if (qrCodeSuccessCallback) {
        ...
        $this.__setHeaderMessage(`Last Match: ${message}`, Html5QrcodeScanner.STATUS_SUCCESS);
      }
    };
  ...
  __renderCameraSelection(cameras) {
    const $this = this;
    const scpCameraScanRegion = document.getElementById(this.__getDashboardSectionCameraScanRegionId());
    ...
    cameraActionStartButton.addEventListener('click', (_) => {
      ...
      $this.html5Qrcode
        .start(cameraId, config, $this.qrCodeSuccessCallback, $this.qrCodeErrorCallback)
        .then((_) => {
  ...
  __resetHeaderMessage() {
    const messageDiv = document.getElementById(this.__getHeaderMessageContainerId());
