...
export default class BenefitSettingAdd extends Component<any, any> {
  ...
  constructor(props) {
    ...
    this.state = {
      loading: false,
      initData: null,
      //  目前该状态用于控制communicationGiftPromotion是否可以编辑
      editable: true
    };
  }
  ...
  componentDidMount() {
    ...
    const { marketingId } =
      this.props.match && this.props.match.params ? this.props.match.params : null;
  ...
  initData = async (marketingId) => {
    ...
    if (res.code == Const.SUCCESS_CODE) {
      // 如果是 communication gift promotion 需要调接口查询状态来控制能否能编辑
      if (res.context.marketingName === 'communication gift promotion') {
        const {
          res: { context }
        } = await getfullGiftState(marketingId);
        //   如果是running 不能编辑
        if (context === 'RUNNING') {
  ...
  getPromotionCode = () => {
    if (!this.state.promotionCode) {
      let randomNumber = (
        '0'.repeat(8) + parseInt(String(Math.pow(2, 40) * Math.random())).toString(32)
      ).slice(-8);
    ...
    form.validateFields((err, values) => {
      ...
      if (!err) {
        ...
        let params = {
          ...
          publicStatus: 1,
          promotionType:
            benefitType === config.CONSUMPTION_GIFT || values.type === 'all'
              ? 0
        ...
        <div className="BenefitSettingAdd-wrap">
          <BreadCrumb thirdLevel={true}>
            <Breadcrumb.Item>
              <FormattedMessage
                id={
                  this.props.benefitType === config.CONSUMPTION_GIFT
                    ? 'Subscription.Consumption gift'
                    : 'Subscription.Welcomebox'
                }
              />
            </Breadcrumb.Item>
          </BreadCrumb>
