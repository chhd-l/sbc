...
export default class ListView extends React.Component<any, any> {
  ...
  render() {
    const { data, loading, onEdit, lastData } = this.props.relaxProps;
    return (
      <DataGrid
        loading={loading}
        rowKey="storeLevelId"
        ...
        pagination={false}
      >
        <Column title="等级名称" key="levelName" dataIndex="levelName" />

        <Column
          title="折扣率"
          key="discountRate"
          dataIndex="discountRate"
          render={(discountRate) => parseFloat(discountRate).toFixed(2)}
        />

        <Column
          title="升级条件"
          render={(rowInfo) => this.renderColumn(rowInfo)}
        />
            ...
            return (
              <div>
                <AuthWrapper functionName="f_customer_level_1">
                  <a href="javascript:void(0);" onClick={() => onEdit(rowInfo)}>
                    设置
                  </a>
                </AuthWrapper>
  ...
  renderColumn = (rowInfo) => {
    let amountConditions = rowInfo.amountConditions
      ? '累计消费金额' + rowInfo.amountConditions + '元'
      : '';
    let orderConditions = rowInfo.orderConditions
      ? '累计支付' + rowInfo.orderConditions + '笔'
      : '';
