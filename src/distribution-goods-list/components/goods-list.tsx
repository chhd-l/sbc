...
export default class GoodsList extends React.Component<any, any> {
  ...
  render() {
    const { brandList, goodsInfoList, total, pageNum, init, sortedInfo, onShowEditCommissionModal, onDelete } = this.props.relaxProps;
    // 表格排序
    ...
    return (
      <div>
        <DataGrid
          ...
          onChange={(pagination, filters, sorter) => this._handleOnChange(pagination, filters, sorter)}
        ...
        >
            ...
            render={(rowInfo) => {
              ...
              return (
                <div key={dataResult.goodsInfoId} style={styles.item}>
                  <div>
                    <img src={dataResult.goodsInfoImg ? dataResult.goodsInfoImg : defaultImg} style={styles.imgItem} />
                  </div>
          ...
          />
          <Column title="店铺分类" dataIndex="storeCateIds" key="storeCateIds" render={this._renderStoreCateList} />
          <Column
            title="品牌"
            dataIndex="brandId"
            key="brandId"
          ...
          />
          <Column title="市场价" dataIndex="marketPrice" key="marketPrice" sorter={true} sortOrder={sortInfo.columnKey === 'marketPrice' && sortInfo.order} render={(marketPrice) => (marketPrice == null ? 0.0 : marketPrice.toFixed(2))} />
          <Column title="佣金比例" dataIndex="commissionRate" key="commissionRate" sorter={true} sortOrder={sortInfo.columnKey === 'commissionRate' && sortInfo.order} render={(commissionRate) => (commissionRate == null ? '0%' : QMFloat.accMul(commissionRate, 100) + '%')} />
          <Column
            title="预估佣金"
            dataIndex="distributionCommission"
            key="distributionCommission"
            sorter={true}
            sortOrder={sortInfo.columnKey === 'distributionCommission' && sortInfo.order}
            render={(distributionCommission) => (distributionCommission == null ? 0.0 : distributionCommission.toFixed(2))}
          />
          <Column title="库存" dataIndex="stock" key="stock" sorter={true} sortOrder={sortInfo.columnKey === 'stock' && sortInfo.order} />
          {/*<Column
            title="分销销量"
          ...
          <Column
            ...
            render={(rowInfo) => {
              const { distributionGoodsAudit, distributionGoodsAuditReason } = rowInfo;
              return (
                <div>
                  <p>{GOODS_AUDIT_TYPE[distributionGoodsAudit]}</p>
                  {(distributionGoodsAudit == 3 || distributionGoodsAudit == 4) && (
                    <Tooltip placement="topLeft" title={distributionGoodsAuditReason}>
                      <a href="#">原因</a>
                    </Tooltip>
                  )}
                </div>
            ...
            render={(rowInfo) => {
              return (
                <div className="operation-box">
                  ...
                  {(rowInfo.distributionGoodsAudit == 2 || rowInfo.distributionGoodsAudit == 3) && (
                    <div>
                      <AuthWrapper functionName={'f_distribution_goods_edit'}>
                        <a style={{ marginRight: 10 }} href="javascript:void(0);" onClick={() => onShowEditCommissionModal(rowInfo)}>
                          编辑
                        </a>
                      </AuthWrapper>
                      ...
                      <AuthWrapper functionName={'f_distribution_goods_del'}>
                        <Popconfirm title="确定删除该商品？" onConfirm={() => onDelete(rowInfo.goodsInfoId, rowInfo.goodsInfoNo)} okText="确定" cancelText="取消">
                  ...
                  {rowInfo.distributionGoodsAudit == 4 && (
                    <div>
                      ...
                      <AuthWrapper functionName={'f_distribution_goods_del'}>
                        <Popconfirm title="确定删除该商品？" onConfirm={() => onDelete(rowInfo.goodsInfoId)} okText="确定" cancelText="取消">
                          <a href="javascript:void(0);">删除</a>
                        </Popconfirm>
                      </AuthWrapper>
                    </div>
  ...
  _dealData = (rowInfo) => {
    ...
    const { goodsInfoId, goodsInfoNo, goodsInfoName, goodsInfoImg, specDetailRelIds } = rowInfo;
  ...
  _handleOnChange = async (pagination, _filters, sorter) => {
    ...
    const { init, onFormFieldChange, setSortedInfo, pageNum, checkSwapInputGroupCompact } = this.props.relaxProps;
    ...
    if (sorter.columnKey && sorter.order && (sorter.columnKey != sortedInfo.columnKey || sorter.order != sortedInfo.order)) {
      currentPage = 1;
