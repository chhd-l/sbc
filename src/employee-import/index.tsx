 ...
 */
const importEmployee = (ext) => {
  return Fetch('/customer/employee/import/' + ext, {
    method: 'GET'
  });
};

export default class EmployeeImport extends React.Component<any, any> {
  constructor(props) {
    super(props);
    this.state = {
      current: 0,
        ...
        <div className="container">
          ...
          <Alert
            ...
            description={
              <ul>
                <li>
                  1、请先下载员工导入模板，并按照批注中的要求填写数据，未按要求填写将会导致导入失败。
                </li>
                <li>
                  2、请选择 .xlsx或
                  .xls文件，文件大小≤2M，每次只能导入一个文件，建议每次导入不超过1000条数据。
                </li>
              </ul>
            }
          />
            ...
            <Spin spinning={this.state.loading} indicator={<img className="spinner" src="https://wanmi-b2b.oss-cn-shanghai.aliyuncs.com/202011020724162245.gif" style={{ width: '90px',height: '90px' }} alt="" />}>
              <div className="steps-content" style={styles.center}>
                <Dragger
                  ...
                  accept=".xls,.xlsx"
                  headers={header}
                  action={Const.HOST + '/customer/employee/excel/upload'}
                ...
                <p style={styles.grey}>
                  请选择 .xlsx或
                  .xls文件，文件大小≤2M，每次只能导入一个文件，建议每次导入不超过500条数据。
                </p>

                {errBtn ? (
                  <Button
                    type="primary"
                    onClick={this._importEmployee}
                    disabled={isImport}
                  >
                    重新导入
                  </Button>
  ...
  toDownTempl() {
    ...
    if (token) {
      ...
      // 新窗口下载
      const exportHref =
        Const.HOST + `/customer/employee/excel/template/${encrypted}`;
  ...
  _importEmployee = async () => {
    ...
    const importRes: any = await importEmployee(ext);
  ...
  toExcel = () => {
    ...
    if (atoken != '') {
      ...
      // 新窗口下载
      const exportHref =
        Const.HOST + `/customer/employee/excel/err/${ext}/${encrypted}`;
      window.open(exportHref);
    } else {
      message.error('请登录');
    }
