...
export default class GoodsList extends React.Component<any, any> {
  ...
  render() {
    const { brandList, goodsInfoList, total, pageNum, init, sortedInfo, onShowEditEnterprisePriceModal, onDelete, iepInfo } = this.props.relaxProps;
    // 表格排序
    ...
    return (
      <div>
        ...
        >
          ...
          {iepSwitch && (
            <Column title={enterprisePriceName} dataIndex="enterPrisePrice" key="enterPrisePrice" sorter={true} sortOrder={sortInfo.columnKey === 'enterPrisePrice' && sortInfo.order} render={(enterPrisePrice) => (enterPrisePrice == null ? 0.0 : enterPrisePrice.toFixed(2))} />
          )}
          ...
          <Column
            ...
            render={(rowInfo) => {
              const { enterPriseAuditState, enterPriseGoodsAuditReason } = rowInfo;
              return (
                <div>
                  <p>{GOODS_AUDIT_TYPE[enterPriseAuditState]}</p>
                  {enterPriseAuditState == 3 && (
                    <Tooltip placement="topLeft" title={enterPriseGoodsAuditReason}>
                      <a href="#">原因</a>
                    </Tooltip>
                  )}
                </div>
            ...
            render={(rowInfo) => {
              return (
                <div className="operation-box">
                  ...
                  {(rowInfo.enterPriseAuditState == 2 || rowInfo.enterPriseAuditState == 3) && (
                    <div>
                      <AuthWrapper functionName={'f_enterprise_goods_edit'}>
                        <a style={{ marginRight: 10 }} href="javascript:void(0);" onClick={() => onShowEditEnterprisePriceModal(rowInfo)}>
                          编辑
                        </a>
                      </AuthWrapper>
