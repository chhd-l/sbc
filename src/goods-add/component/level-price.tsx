...
class LevelPriceForm extends React.Component<any, any> {
            ...
            <CustomerSPU>
              <FormItem label="SPU unified market price">
                ...
                <Tooltip
                  placement="top"
                  title={
                    '按照客户设价时需要填写统一的市场价，保存后，原有的SKU市场价将会被覆盖'
                  }
                >
                  <a style={{ fontSize: 14 }}>
                  ...
                  return (
                    <FormItem>
                      {getFieldDecorator('levelcount_' + levelId, {
                        ...
                        <InputNumber
                          disabled={index > 0 && levelCountDisable}
                        />
                      )}
                    </FormItem>
                  );
  ...
  _synchLevelCount = (e) => {
    ...
    // 是否同步值
    if (e.target.checked) {
      let count = userLevelPrice.get('0')
        ? userLevelPrice.get('0').get('count')
        : '';
      // form表单initialValue方式赋值不成功，这里通过setFieldsValue方法赋值
      const fieldsValue = this.props.form.getFieldsValue();
      // 同步值
  ...
  _synchLevelMaxCount = (e) => {
    ...
    if (e.target.checked) {
      let count = userLevelPrice.get('0')
        ? userLevelPrice.get('0').get('maxCount')
        : '';
      // form表单initialValue方式赋值不成功，这里通过setFieldsValue方法赋值
      const fieldsValue = this.props.form.getFieldsValue();
      // 同步值
