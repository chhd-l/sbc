...
import { sendEmail } from '@/myvetreco-logins/forget-password/webapi';
import moment from 'moment';
import '../css/login-form.less';

const bgImg = require('../img/login--forget.jpg');

export default Form.create()(class LoginForgetForm extends React.Component<any, any> {
      ...
      if (!err) {
        ...
        sendEmail(values.email).then(data => {
          if (data.res.code === Const.SUCCESS_CODE) {
            history.push({ pathname: '/reset-step2', state: { email: values.email } });
          } else {
        ...
        <div style={{textAlign:'center'}}>
          <div>
            <div style={{marginBottom: 20, fontSize: 30}}>
              <span style={{color: 'var(--primary-color)'}}>Забыли пароль?</span>
            </div>
            <div style={{marginBottom: 79, fontSize: 18}}>
              <span style={{color: '#666'}}>Введите адрес электронной почты, связанный с вашей учетной записью.</span>
            </div>
            <Form layout="vertical" style={{width: 270, margin: '0 auto', textAlign: 'initial'}}>
              <Form.Item>
                <div className={`form-item-label ${this.state.emailActive ? 'placeholder' : ''}`}>Адрес электронной почты</div>
                {getFieldDecorator('email', {
                  initialValue: '',
                  rules: [{ type: 'email', message: 'Адрес электронной почты должен быть указан в правильном формате.' }]
                })(<Input autoComplete="off" onFocus={this.handleInputFocus} onBlur={this.handleInputBlur} />)}
              </Form.Item>
            </Form>
            <div>
              <Button type="primary" className="login-button" loading={this.state.loading} onClick={this.handleLogin}>Отправить ссылку для авторизации</Button>
              <span style={{marginLeft: 10}}>или</span>
              <Link to="/login-form" className="link-b" style={{marginLeft: 10}}>Отменить</Link>
            </div>
          </div>
