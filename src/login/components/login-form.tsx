...
const bgImg = require('../img/login--backgroud.png');

export default Form.create()(class LoginForm extends React.Component<any, any> {
  ...
  componentDidMount(): void {
    ...
    setFieldsValue({ account: localStorage.getItem('remember_account') || '' });
  }
  ...
  handleInputBlur = (key: string) => {
    if (key === 'email' && this.props.form.getFieldValue('account') === '') {
      this.setState({ emailActive: true });
    } else if (key === 'password' && this.props.form.getFieldValue('password') === '') {
    ...
    this.props.form.validateFields(null, (err, values) => {
      if (!values.account || !values.password) {
        ...
        if (this.state.remember) {
          localStorage.setItem('remember_account', values.account);
        } else {
          localStorage.setItem('remember_account', '');
        }
        login(values, '', () => {
      ...
      <div className="spec-login-form">
        ...
        <div className="login-area">
          <div className="login-left">
            <div style={{marginBottom: 60, fontSize: 30}}>
              <span style={{color: '#666'}}>Войдите в</span>
              <span style={{color: 'var(--primary-color)'}}> свою учетную запись</span>
            </div>
            <Form layout="vertical">
              {this.state.hasError ? <div style={{marginBottom: 50}}>
                <Alert
                  message="Пожалуйста, проверьте имя пользователя и пароль и попробуйте еще раз."
                  closable
                  afterClose={() => this.setState({hasError: false})}
                />
              </div> : null}
              <Form.Item>
                <div className={`form-item-label ${this.state.emailActive ? 'placeholder' : ''}`}>Адрес электронной почты</div>
                {getFieldDecorator('account', {
                  initialValue: ''
                })(<Input autoComplete="off" onFocus={() => this.handleInputFocus('email')} onBlur={() => this.handleInputBlur('email')} />)}
              </Form.Item>
              <Form.Item>
                <div className={`form-item-label ${this.state.passwordActive ? 'placeholder' : ''}`}>Введите пароль</div>
                {getFieldDecorator('password', {
                  initialValue: ''
                })(<Input.Password autoComplete="off" onFocus={() => this.handleInputFocus('password')} onBlur={() => this.handleInputBlur('password')} />)}
              </Form.Item>
            </Form>
            <div style={{marginBottom: 50}}>
              <Checkbox checked={this.state.remember} onChange={(e) => this.setState({ remember: e.target.checked })}><span style={{fontSize: 16}}>Запомнить меня</span></Checkbox>
              <Link to="/reset-step1" className="link-b">Забыли пароль</Link>
            </div>
            <div>
              <Button type="primary" className="login-button" loading={this.state.loading} onClick={this.handleLogin}>Войти</Button>
              <span style={{marginLeft: 10}}>или</span>
              <Link to="/login" className="link-b" style={{marginLeft: 10}}>Отменить</Link>
            </div>
          </div>
