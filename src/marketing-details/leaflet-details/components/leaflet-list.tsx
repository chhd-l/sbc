...
@withRouter
@Relax
export default class LeafletList extends React.Component<any, any> {
  props: {
    history?: any;
    relaxProps?: {
      leafletLevelList: IList;
      leafletList: IList;
      brands: IList;
      cates: IList;
      subType: any;
  ...
  static relaxProps = {
    leafletLevelList: 'leafletLevelList',
    leafletList: 'leafletList',
  ...
  render() {
    const { leafletLevelList, leafletList, brands, cates, subType } = this.props.relaxProps;

    let skuList = fromJS(leafletList);
    let brandList = fromJS(brands ? brands : []);
    let cateList = fromJS(cates ? cates : []);
    let dataSource = leafletLevelList.map((level) => {
      level = level.set(
        'fullLeafletDetailList',
        level.get('fullLeafletDetailList').map((gift) => {
              ...
              <Row>
                <Col span={24}>
                  ...
                  <Col span={18}>
                    ...
                    {subType == '15' ? sessionStorage.getItem(cache.SYSTEM_GET_CONFIG) : 'Items'} {level.giftType == '1' ? <FormattedMessage id="Marketing.AnOptionalOne" /> : <FormattedMessage id="Marketing.TheDefaultAllGive" />}
                  </Col>
                </Col>
              </Row>
            </GreyBg>

            <Table dataSource={level.fullLeafletDetailList} pagination={false} rowKey="giftDetailId">
