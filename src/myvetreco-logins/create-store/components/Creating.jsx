...
export default function Creating({ userInfo, setStep }) {
  ...
  const list = statusList.map((item, index) => ({
    name: <FormattedMessage id={`Login.create_store_step${index + 1}`} />,
    top: -((index + 1) * 31),
    result: item.result
  }));
        ...
        if (errorIndex == 9) {
          ...
          finishCreateStore({
            ...
            if (res.code === 'K-000000') {
              //标识第一次登录
              sessionStorage.setItem('myvet-isFirstLogin', true);
  ...
  const statusIcon = (status) => {
    switch (status) {
      case 'pending':
        return (
          <span style={{ color: 'var(--primary-color)', paddingLeft: 5 }}>
            <Icon type="loading" />
          </span>
        );
      case false:
        return (
          <span style={{ color: '#ff7875', paddingLeft: 5 }}>
            <Icon type="close" />
          </span>
        );
      case true:
        return (
          <span style={{ color: '#3bff00', paddingLeft: 5 }}>
            <Icon type="check" />
          </span>
        );
    }
    ...
    <div className="create-transition">
      ...
      <div className="step-list">
        <h1 style={{ color: '#30465a', minWidth: 400 }}>
          {classText === '' ? (
            <FormattedMessage id={`Login.create_store_ing`} />
          ) : classText === 'ok' ? (
            <FormattedMessage id="Login.create_store_success" />
          ) : classText === 'finished' ? (
            <FormattedMessage id="Login.loging" />
          ) : (
            <FormattedMessage id="Login.create_store_error" />
          )}
          {/*{*/}
