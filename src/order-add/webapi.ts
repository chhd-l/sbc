 ...
 * @returns {Promise<IAsyncResult<TList>>}
 */
export const fetchCustomer = (customerAccount) => {
  const request = {
    method: 'POST',
    body: JSON.stringify({ customerAccount: customerAccount })
  };
  return Fetch<TResult>('/customer/customerAccount/list', request);
};
...
export const remedyOrder = (params) => {
  return Fetch<TResult>('/trade/remedy-part', {
    method: 'PUT',
    body: JSON.stringify({
      ...params
    })
...
export const fetchSingleCustomer = (customerId) => {
  return Fetch(`/customer/bySupplier/${customerId}`);
};
