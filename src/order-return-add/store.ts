...
export default class AppStore extends Store {
  ...
  init = async (tid: string) => {
    ...
    // 可退最大金额
    let canApplyPrice = tradeDetail.res.context.canReturnPrice;
    // 已申请退货订单
    let returnOrderList = [];
  ...
  _setReturnGifts = () => {
    ...
    if (tradeMarketings && tradeMarketings.size > 0) {
      ...
      if (giftMarketings && giftMarketings.size > 0) {
        ...
        const currReturnSkus = trade.get('tradeItems').filter((item) => item.get('num') > 0); //本次需要退的商品汇总
        ...
        giftMarketings.forEach((giftMarketing) => {
          if (4 == giftMarketing.get('subType')) {
            ...
            if (leftSkuAmount < giftMarketing.get('giftLevel').get('fullAmount')) {
              allReturnGifts = this._setReturnGiftsMap(allReturnGifts, giftMarketing);
            }
