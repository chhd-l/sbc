...
import { getCateList, addCate, deleteCate, editCate, getImage, getChild } from './webapi';
...
export default class AppStore extends Store {
   ...
   * 检测图片分类是否有子类图片
   */
  validImage = async (cateId: string) => {
    const result: any = await getImage(Map({ cateId: cateId }));
    if (result.res.context == 0) {
      this.dispatch('cateActor: image', false);
    }
    if (result.res.context == 1) {
      this.dispatch('cateActor: image', true);
    }
  };
