...
export default class AppStore extends Store {
  ...
  init = async (goodsId?: string) => {
    // 保证品牌分类等信息先加载完
    await Promise.all([getCateList(), getBrandList(), checkSalesType(goodsId), isFlashsele(goodsId), getStoreCateList(), getDetailTab(), this.onRelatedList(goodsId)]).then((results) => {
  ...
  saveMain = async () => {
    ...
    const tabs = [];
    if (data.get('detailEditor0') && data.get('detailEditor0').val && data.get('detailEditor0').val.getContent) {
      tabs.push({
    ...
    if (parseInt(goods.get('subscriptionStatus')) === 1 && goodsList.toJS().filter((item) => item['subscriptionStatus'] === 1).length === 0) {
