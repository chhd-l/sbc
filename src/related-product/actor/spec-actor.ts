...
export default class GoodsSpecActor extends Actor {
   ...
   */
  _randomGoodsInfoNo() {
    const skuNo = '8' + new Date(sessionStorage.getItem('defaultLocalDateTime')).getTime().toString().slice(4, 10) + (window.crypto.getRandomValues(new Uint8Array(1)) * 0.001).toString().slice(2, 5);
