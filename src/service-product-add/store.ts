...
export default class AppStore extends Store {
      ...
      (results) => {
        if ((results[0].res as any).code === Const.SUCCESS_CODE) {
          this.transaction(() => {
            ...
            this.dispatch(
              'goodsActor:serviceTypeList',
              (results[0].res as any).context.service_type?.goodsDictionaryPage?.content ?? []
            );
            this.dispatch(
              'goodsActor:expertTypeList',
              (results[0].res as any).context.expert_type?.goodsDictionaryPage?.content ?? []
            );
  ...
  initServiceSpec = () => {
    ...
    [
      { id: 100001, name: 'Duration' },
      { id: 100002, name: 'Expert type' }
    ].forEach((item) => {
  ...
  _getGoodsDetail = async (resource) => {
    ...
    this.transaction(() => {
      ...
      if (
        !tmpContext.goodsDescriptionDetailList ||
        tmpContext.goodsDescriptionDetailList.length === 0
      ) {
      ...
      item.get('specValues').forEach((specValueItem) => {
        goodsSpecDetails = goodsSpecDetails.push(
          Map({
            ...
            mockSpecDetailId:
              specValueItem.get('mockSpecDetailId') || specValueItem.get('specDetailId'),
