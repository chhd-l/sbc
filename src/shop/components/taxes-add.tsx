    ...
    webApi
      ...
      .then((data) => {
        ...
        if (res.code === Const.SUCCESS_CODE) {
          message.success(res.message || (window as any).RCi18n({ id: 'Setting.Operationsuccessful' }));
          this.props.closeFunction(true);
        } else {
          message.error(res.message || (window as any).RCi18n({ id: 'Setting.Operationfailure' }));
        }
      })
      .catch((err) => {
        message.error(err.toString() || (window as any).RCi18n({ id: 'Setting.Operationfailure' }));
      });
  };
  ...
  render() {
    ...
    return (
      <Modal
        width={600}
        maskClosable={false}
        title={isEdit ? (window as any).RCi18n({ id: 'Setting.Edittaxzone' }) : (window as any).RCi18n({ id: 'Setting.Newtaxzone' })}
        visible={visible}
        ...
        footer={[
          ...
          >
            {(window as any).RCi18n({ id: 'Setting.Cancel' })}
          </Button>,
          <Button key="submit" type="primary" onClick={this.handleSubmit}>
            {(window as any).RCi18n({ id: 'Setting.Submit' })}
          </Button>
        ]}
      >
        <Form {...formItemLayout}>
          <FormItem label={(window as any).RCi18n({ id: 'Setting.Taxzonename' })}>
            {getFieldDecorator('taxZoneName', {
              rules: [
                { required: true, message: (window as any).RCi18n({ id: 'Setting.Taxzonenameisrequired' }) },
                {
                  max: 50,
                  message: (window as any).RCi18n({ id: 'Setting.Exceedmaximumlength' })
                }
              ],
          ...
          <FormItem label={(window as any).RCi18n({ id: 'Setting.Taxzonedescription' })}>
          ...
          <FormItem label={(window as any).RCi18n({ id: 'Setting.Taxzonetype' })}>
            {getFieldDecorator('taxZoneType', {
              rules: [{ required: true, message: (window as any).RCi18n({ id: 'Setting.Taxzonetypeisrequired' }) }],
          ...
          <FormItem label={(window as any).RCi18n({ id: 'Setting.Zoneincludes' })}>
            {getFieldDecorator('zoneIncludes', {
              rules: [
                {
                  required: true,
                  message: (window as any).RCi18n({ id: 'Setting.PleaseSelectzone' })
                }
              ],
              ...
              <Select
                style={{ width: '80%' }}
                showSearch
                placeholder={(window as any).RCi18n({ id: 'Setting.Selectzone' })}
          ...
          <FormItem label={(window as any).RCi18n({ id: 'Setting.Taxrates' })}>
            {getFieldDecorator('taxRates', {
              rules: [{ required: true, message: (window as any).RCi18n({ id: 'Setting.Taxratesisrequired' }) }],
