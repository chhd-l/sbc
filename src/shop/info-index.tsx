...
export default class ShopInfo extends React.Component<any, any> {
  ...
  render() {
    ...
    return (
      <div>
        <Header preTxt={header.get('preTxt')}
        ...
        <div className="shopContent">
          <h1>{header.get('btnShow') ? '开店申请审核未通过' : '开店申请审核中'}</h1>
          <Steps current={currentStep}>
            <Step title="基本信息" onClick={() => this.store.setCurrentStep(0)}></Step>
            <Step title="工商信息" onClick={() => this.store.setCurrentStep(1)}></Step>
            <Step title="签约信息" onClick={() => this.store.setCurrentStep(2)}></Step>
            <Step title="财务信息" onClick={() => this.store.setCurrentStep(3)}></Step>
          </Steps>
