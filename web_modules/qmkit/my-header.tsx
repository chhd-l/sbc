   ...
   content=() => (
    <div style={{ width: 350,minHeight: 380 }}>
      <Tabs defaultActiveKey="1" >
              ...
              return (
                <li style={styles.popoverList} className="popover-list" key={item.id}>
                  <div className="popover-list-text" style={styles.popoverListText}>
                    <div style={{ color: 'red', cursor: 'pointer',fontSize:16 }} onClick={() => this.returnTask(item,'reminderTasks')}>{item.title}</div>
        ...
        <ul style={{height: 380,  overflow: 'auto' }}>
            {this.state.taskList.length > 0 ? this.state.taskList.map(item => {
              return (
                <li style={styles.popoverList} className="popover-list" key={item.id}>
                  <div style={{ width: '15%', textAlign: 'center' }} > <img src={text} alt="" /> </div>
      ...
      <div style={styles.linkMore}>
        <Link to="/tasks" style={{ fontSize: 16,color:'#909399' }}>View all task</Link>
        <Icon type="right" />
      </div>
    </div>
  );
